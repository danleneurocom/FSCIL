...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        # FIX: Sum features instead of concatenating to maintain common_dim
        # This resolves the dimension mismatch error
...
def train_patchcore_fast_slow(train_loader, feature_extractor, num_epochs=10, temperature=0.1):
    """
    Trains the fast and slow prompts using a contrastive loss.
    """
    feature_extractor.train()
    feature_extractor.to(DEVICE)
    
    fast_optimizer = torch.optim.Adam([feature_extractor.fast_prompt_embeddings], lr=1e-3)
    slow_optimizer = torch.optim.Adam([feature_extractor.slow_prompt_embeddings], lr=1e-5)
    
    for epoch in tqdm(range(num_epochs), desc="Training Prompts"):
        for batch in train_loader:
            ...
            # Separate prompts from patch features
            fast_prompts = patch_features_full[:, :16, :]
            slow_prompts = patch_features_full[:, 16:32, :]
            patch_features = patch_features_full[:, 32:, :]
            
            # Reshape features for contrastive loss calculation
            prompts = torch.cat([fast_prompts, slow_prompts], dim=1)
            all_features = torch.cat([prompts.reshape(-1, prompts.shape[-1]), patch_features.reshape(-1, patch_features.shape[-1])], dim=0)
            ...
            # Create a mask for positive pairs (prompts are positive for all patches in the batch)
            # This is a simplification. A more robust loss would involve memory banks.
            # But this is a good first step to train the prompts.
            num_prompts = prompts.shape[1]
            num_patches = patch_features.shape[1]
            
            labels = torch.arange(len(all_features)).to(DEVICE)
            
            # Positive pairs: patches to prompts, and vice versa
            positive_mask = torch.zeros_like(similarity_matrix, dtype=torch.bool).to(DEVICE)
            # Prompts are positive to all patches in the same batch
            ...
            # Denominator: sum over all other features (including self)
            denominator = torch.sum(exp_similarity, dim=1, keepdim=True)
            
            # Numerator: sum over positive pairs
            numerator = torch.sum(exp_similarity * positive_mask, dim=1)
            
            # Loss = -log(numerator / denominator)
            total_loss = -torch.log(numerator / denominator.squeeze()).mean()
    ...
    # Build the final memory bank after training the prompts
    memory_bank = []
    ...
    with torch.no_grad():
        for batch in tqdm(train_loader, desc="Building Memory Bank"):
            images = batch["image"].to(DEVICE)
            patch_features_full = feature_extractor(images)
            # Take only the patch features, not the prompts
            patch_features = patch_features_full[:, 32:, :]
...
def test_patchcore_sam(
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing"):
            ...
            patch_features_full = feature_extractor(images)  # (B, Np, C)
            
            # New: Discard prompt features before distance calculation
            patch_features = patch_features_full[:, 32:, :] 
...
if __name__ == '__main__':
    ...
    # --- Run Training ---
    # New: Use the fast and slow training function
    memory_bank = train_patchcore_fast_slow(train_loader, feature_extractor)
