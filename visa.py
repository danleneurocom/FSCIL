...
# --------------------------------------------------------------------------------------
# Dataset: VisA (IC industrial AD). Flexible test/mask discovery.
# Directory assumptions (common in released VisA zips):
#   <root>/<category>/
#       train/good/*.png|jpg|jpeg|bmp|tif|tiff
#       test/good/*.*
#       test/<defect_name>/*.*
#   Optional masks (any of these names): ground_truth/, masks/, mask/, annotations/
#       <root>/<category>/<mask_root>/<defect_name>/<same_stem>*.png (binary)
# If masks are absent, pixel metrics still run (maps are compared to zero masks).
# --------------------------------------------------------------------------------------
...
class VisA(Dataset):
    def __init__(
        ...
        """
        Expected layout:
          <root>/<category>/Data/Images/Normal/*.png|jpg|...
                                     /Anomaly/<anything>.* (same flat or subdirs OK)
                                     /Mask/<same-stem-as-anomaly>.*  (PNG recommended)
          <root>/<category>/image_anno.csv (optional; not required)
        """
        self.root = Path(root)
        ...
        img_root = base / "Data" / "Images"
        normal_dir  = img_root / "Normal"
        anomaly_dir = img_root / "Anomaly"
        mask_dir    = img_root / "Mask"

        # be forgiving: no hard asserts beyond the top-level Images
        if not img_root.exists():
            raise AssertionError(f"[VisA] Missing: {img_root}")
        if split == "train" and not normal_dir.exists():
            raise AssertionError(f"[VisA] Missing Normal dir: {normal_dir}")
        if split == "test" and (not normal_dir.exists() or not anomaly_dir.exists()):
            raise AssertionError(f"[VisA] Missing Normal/Anomaly dirs under: {img_root}")

        # gather files
        exts = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff"}
        def all_imgs(d: Path) -> List[Path]:
        ...
        if split == "train":
            # Only NORMAL images → label=0
            for p in sorted(all_imgs(normal_dir)):
                img_paths.append(p)
            ...
            for p in sorted(all_imgs(normal_dir)):
                ...
                labels.append(0)
                defect_types.append("good")
            # ANOMALY images → label=1 (+ optional mask)
            for p in sorted(all_imgs(anomaly_dir)):
                img_paths.append(p)
                labels.append(1)
                # try to find a mask with the same stem anywhere under Mask/
                mp = None
                if mask_dir.exists():
                    cand = list(mask_dir.rglob(p.stem + ".*"))
                    if len(cand) == 0:
                        # sometimes masks keep full name with extension replaced by .png
                        cand = list(mask_dir.rglob(p.name.replace(p.suffix, ".png")))
                    mp = cand[0] if len(cand) > 0 else None
                mask_paths.append(mp)
                # defect_type: take parent folder name under Anomaly if present, else "anom"
                try:
                    dt = p.relative_to(anomaly_dir).parts[0]
                except Exception:
                    dt = "anom"
...
def load_visa_category(
    ...
    train_ds = VisA(root, category, split="train", image_size=image_size)
    test_ds  = VisA(root, category, split="test",  image_size=image_size)
...
def get_continual_test_loaders_generic(
    seen_categories: List[str],
    data_root: str,
    image_size: int,
    batch_size: int,
    dataset_kind: str = "mvtec",  # 'mvtec' or 'visa'
):
    if dataset_kind == "mvtec":
        ds_list = [MVTecAD(data_root, cat, split="test", image_size=image_size) for cat in seen_categories]
    elif dataset_kind == "visa":
        ds_list = [VisA(data_root, cat, split="test", image_size=image_size) for cat in seen_categories]
    else:
        raise ValueError(f"Unknown dataset_kind: {dataset_kind}")
    combined_test_ds = torch.utils.data.ConcatDataset(ds_list)
    return DataLoader(
        combined_test_ds, batch_size=batch_size, shuffle=False,
        num_workers=0, pin_memory=False, collate_fn=test_collate_fn
    )
...
# --- helper: UCAD top-k resolver (count or ratio) ---
def _resolve_topk_ucad(num_items: int, topk: int | float, mode: str = "count") -> int:
    """
    mode='count' : interpret topk as an absolute count (e.g., 100)
    mode='ratio' : interpret topk in (0,1] as a fraction of num_items
    Always clamps to [1, num_items].
    """
    if mode == "ratio":
        if isinstance(topk, float) and 0.0 < topk <= 1.0:
            k = int(math.ceil(num_items * topk))
        else:
            k = int(topk)
    else:  # 'count'
        k = int(round(float(topk)))
...
@torch.no_grad()
def select_task_by_imgscore_ucad(
    extractor: ViTPromptExtractor,
    ucad_mem: UCADMemory,
    imgs: torch.Tensor,
    image_topk: int | float = 100,
    pooling_mode: str = "count",      # 'count' (k is absolute) or 'ratio' (k = fraction of N)
    knn_k: int = 1,                   # 1 = UCAD min-NN; >1 = kNN mean
    metric: str = "euclidean",        # 'euclidean' or 'cosine'
    use_multiscale: bool = False,     # False = UCAD apples-to-apples
    key_layer: int = 5,
    center_quantile: float | None = None,
) -> List[str]:
    """
    Route each image to the task with the *lowest raw image anomaly score*
    computed with the CURRENT model (no prompt-bank loads).
    """
    extractor.eval().to(DEVICE)
    B, _, H, W = imgs.shape
    ps = 16
    ...
    best_vals = [float("inf")] * B
    best_tasks = [None] * B

    for t in ucad_mem.tasks():
        Kf = ucad_mem.mem[t].knowledge.to(Zf.device)        # [Mf, D]
        if Kf.numel() == 0:
            continue

        Df = _pairwise_dist(Zf, Kf, metric=metric).reshape(B_, Nf, -1)   # [B, Nf, Mf]
        if knn_k > 1:
            k_eff = min(knn_k, Df.shape[-1])
            min_final = torch.topk(Df, k=k_eff, dim=2, largest=False).values.mean(dim=2)  # [B,Nf]
        else:
            min_final = Df.min(dim=2).values

        if use_multiscale:
            Ke = ucad_mem.mem[t].keys.to(Zf.device)         # [Me, D]
            if Ke.numel() > 0:
                De_ = _pairwise_dist(Ze, Ke, metric=metric).reshape(B, Ne, -1)  # [B,Ne,Me]
                if knn_k > 1:
                    k_eff = min(knn_k, De_.shape[-1])
                    min_early = torch.topk(De_, k=k_eff, dim=2, largest=False).values.mean(dim=2)  # [B,Ne]
                else:
                    min_early = De_.min(dim=2).values
                # align token counts (pad/crop) then fuse by max
                if Ne != Nf:
                    if Ne > Nf: min_early = min_early[:, :Nf]
                    else:       min_early = F.pad(min_early, (0, Nf - Ne), value=float(min_early.mean()))
                amap = torch.maximum(min_final, min_early)   # [B,N]
            else:
                amap = min_final
        else:
            amap = min_final
        ...
        for b in range(B):
            ...
            score = torch.topk(a, k=k, largest=True).values.mean().item()
            if score < best_vals[b]:
                best_vals[b] = score
                best_tasks[b] = t
...
def infer_batch_anomaly_ucad(
    extractor: ViTPromptExtractor,
    prompt_bank: PromptBank,            # kept for API symmetry
    ucad_mem: UCADMemory,
    imgs: torch.Tensor,
    key_layer: int = 5,
    image_topk: int | float = 100,      # count (e.g., 100) or ratio in (0,1]
    pooling_mode: str = "count",        # 'count' or 'ratio'
    calib_stats: Optional[Dict[str, Dict[str, float]]] = None,  # {'task': {'mu','sigma'}}
    knn_k: int = 1,
    ...
    Flexible UCAD inference (still single-scale by default):
      - Route by current-model image score (same settings you pass in).
      - Pixel map from final layer; if multiscale=True, fuse(min_final,max with early).
    ...
    tasks = select_task_by_imgscore_ucad(
        ...
        knn_k=knn_k, metric=metric, use_multiscale=use_multiscale,
    ...
    for b in range(B):
        ...
        image_scores_out.append(float(img_s.item()))
...
# def infer_batch_anomaly_ucad(
#     extractor: ViTPromptExtractor,
#     prompt_bank: PromptBank,          # kept for API symmetry; NOT used here
#     ucad_mem: UCADMemory,
#     imgs: torch.Tensor,
#     key_layer: int = 5,               # ignored in UCAD eval (no multiscale/keys)
#     image_topk: float | int = 100,    # UCAD default: top-100 (absolute)
#     center_quantile: float | None = None,  # keep None for UCAD
#     calib_stats: Optional[Dict[str, Dict[str, float]]] = None,  # MUST be None in UCAD
#     knn_k: int = 1,                   # UCAD uses k=1 for patch matching
#     use_multiscale: bool = False,     # MUST be False in UCAD
#     metric: str = "euclidean",        # UCAD uses L2
#     pooling_mode: str = "count",      # "count" -> absolute k; "ratio" is NOT UCAD
# ) -> Tuple[torch.Tensor, torch.Tensor]:
#     """
#     UCAD apples-to-apples inference:
#       - For each image, compute final-layer patch tokens ONCE with the CURRENT model.
#       - For every task's knowledge K_n, compute per-patch min distance (or kNN mean if knn_k>1).
#       - Image score = mean of top-k patches (absolute k=100 by default).
#       - Route to the task with MIN image score (argmin over tasks).
#       - Return that min image score and the corresponding pixel map (from that task), no calibration.
#     """
#     assert calib_stats is None, "UCAD eval must NOT use calibration."
#     assert not use_multiscale, "UCAD eval must NOT use multiscale/early-layer fusion."
#     extractor.eval().to(DEVICE)
...
#         # final-layer (prompted) patches using CURRENT prompts (single head)
#         z_final = extractor.get_final_patches(x, use_prompts=True)   # [1, N, D]
#         N, D = z_final.shape[1], z_final.shape[2]
#         Z = z_final.reshape(N, D)                                    # [N, D]

#         best_score = float("inf")
#         best_map: Optional[torch.Tensor] = None

#         # evaluate against EACH task's knowledge with the SAME current model
#         for t in ucad_mem.tasks():
#             K = ucad_mem.mem[t].knowledge.to(Z.device)  # [M, D]
#             if K.numel() == 0:
#                 continue

#             Dmat = _pairwise_dist(Z, K, metric=metric)  # [N, M]

#             # per-patch min (or kNN mean) distance to task knowledge
#             if knn_k > 1:
#                 k_eff = min(knn_k, Dmat.shape[1])
#                 min_d = torch.topk(Dmat, k=k_eff, dim=1, largest=False).values.mean(dim=1)  # [N]
#             else:
#                 min_d = Dmat.min(dim=1).values  # [N]

#             # UCAD image score: mean of absolute top-k patch distances (k=100)
#             k = _resolve_topk_ucad(N, image_topk, mode=pooling_mode)
#             img_score = torch.topk(min_d, k=k, largest=True).values.mean()

#             # keep the task with the lowest image score
#             s_val = float(img_score.item())
#             if s_val < best_score:
#                 best_score = s_val
#                 # reshape to patch grid for the chosen task's pixel map
#                 if N != Hp * Wp:
#                     # pad/trim safely if needed
#                     vec = min_d
#                     if N > Hp * Wp:
#                         vec = vec[:Hp * Wp]
#                     else:
#                         vec = F.pad(vec, (0, Hp * Wp - N), value=float(vec.mean()))
#                     best_map = vec.reshape(Hp, Wp).detach().cpu()
#                 else:
#                     best_map = min_d.reshape(Hp, Wp).detach().cpu()

#         # Fallback if no tasks (shouldn't happen once memory is filled)
#         if best_map is None:
#             best_map = torch.zeros((Hp, Wp), dtype=torch.float32)

#         image_scores_out.append(best_score)
#         maps_out.append(best_map)
...
def _fm_paper_eq7(history_rows: List[List[float]]) -> float:
    """
    UCAD Eq.7 style forgetting:
      For each step i>0, for each earlier task j<i:
        f_{i,j} = max_{k < i} R_{k,j} - R_{i,j}
      FM_i = mean_j f_{i,j} over valid j
    We then report the mean of FM_i over i=1..T-1 (ignoring NaNs).
    *Negative values are kept* (no clamping) exactly as in the paper-style definition.
    """
    T = len(history_rows)
    if T <= 1:
        return 0.0
    ...
    fm_vals = []
    for i in range(1, T):
        row_i = history_rows[i]
        per_task_drops = []
        for j in range(min(i, len(row_i))):
            now = row_i[j] if j < len(row_i) else float('nan')
            prev_vals = []
            for k in range(i):  # 0..i-1
                if j < len(history_rows[k]):
                    prev_vals.append(history_rows[k][j])
            if not prev_vals or np.all(np.isnan(prev_vals)) or np.isnan(now):
                continue
            prev_best = np.nanmax(prev_vals)
            per_task_drops.append(prev_best - now)   # keep sign (can be negative)
        if per_task_drops:
            fm_vals.append(np.nanmean(per_task_drops))
    return float(np.nanmean(fm_vals)) if fm_vals else 0.0


def _fm_clamped(history_rows: List[List[float]]) -> float:
    """
    Same as above but clamps each drop at 0: max(0, prev_best - now).
    Often used in CL papers to avoid negative forgetting.
    """
    T = len(history_rows)
    if T <= 1:
        return 0.0
    ...
    for i in range(1, T):
        ...
        for j in range(min(i, len(row_i))):
            ...
            if not prev_vals or np.all(np.isnan(prev_vals)) or np.isnan(now):
                continue
            prev_best = np.nanmax(prev_vals)
            per_task_drops.append(max(0.0, prev_best - now))
        if per_task_drops:
            fm_vals.append(np.nanmean(per_task_drops))
    return float(np.nanmean(fm_vals)) if fm_vals else 0.0


def _true_category_from_path(p: str, known_categories: Optional[List[str]] = None) -> str:
    parts = [q for q in Path(p).parts if q not in {"train", "test", "ground_truth", "masks", "mask", "annotations"}]
    if known_categories:
        s = set(known_categories)
        for q in parts:
            if q in s:
                return q
    # fallback: the directory just above 'test' or 'train'
    pr = Path(p)
    for i in range(len(pr.parts)-1, -1, -1):
        if pr.parts[i] in {"test", "train"} and i-1 >= 0:
            return pr.parts[i-1]
    # last resort
...
def run_ucad(
    ...
    pooling_mode: str = "count",          # 'count' or 'ratio'
    image_topk: int | float = 100,        # 100 (UCAD) or e.g. 0.02 for 2% when mode='ratio'
    knn_k: int = 1,                       # 1 (UCAD) or >1 for kNN mean
    use_calibration: Optional[str] = None,# None | 'zscore' | 'robust'
    use_multiscale: bool = False,         # False = UCAD apples-to-apples
    metric: str = "euclidean",            # 'euclidean' or 'cosine'
    center_quantile: float | None = None, # optional baseline subtraction before pooling
    ...
    for task_idx, category in enumerate(categories):
        ...
        if dataset == "mvtec":
            ...
            train_ds, _, train_loader, _ = load_visa_category(
                root=data_root, category=category, image_size=image_size, batch_size=batch_size
            )
        else:
            raise ValueError(f"Unknown dataset: {dataset}")

        # split support/query from GOOD images
        ...
        # inner (fast)
        inner_losses = [meta.inner_update(b) for _, b in zip(range(meta.cfg.inner_steps), support_loader)]
        print(f"[inner] mean loss: {np.mean(inner_losses):.5f} (steps={len(inner_losses)})")

        # outer (slow) with replay of previous knowledge
        prev_knowledge = ucad_mem.concat_knowledge()
        outer_stats = meta.outer_update_many(query_loader, prev_knowledge)
        print(f"[outer] loss_q={outer_stats['loss_q']:.5f} | align={outer_stats['loss_align']:.5f} "
              f"| scl={outer_stats['scl']:.5f} | steps={outer_stats['steps']}")

        # snapshot prompts for this task (not used in UCAD eval, but kept)
        prompt_bank.save(category, extractor)

        # build per-task memory (keys/knowledge)
        keys, knowledge = build_task_memory(
            extractor, train_loader, key_layer=meta.cfg.key_layer,
            max_keys=max_keys, max_knowledge=max_knowledge
        )
        ucad_mem.add(category, keys, knowledge)
        print(f"[memory] {category}: keys={keys.shape[0]} | knowledge={knowledge.shape[0]}")

        # optional per-task calibration on train-good
        if use_calibration in {"zscore", "robust"}:
            ...
            if mu is not None:
                ...
                print(f"[calib] {category}: mode={use_calibration} mu={mu:.4f} sigma={sigma:.4f} thr={thr:.4f}")
            else:
                print(f"[calib] {category}: skipped")

        # eval on all seen categories
        print(f"[eval] router=imgscore_ucad(mode={pooling_mode}, k={image_topk}); "
              f"knn_k={knn_k}; calibration={'None' if not use_calibration else use_calibration}; "
              f"multiscale={use_multiscale}; metric={metric}")
        test_loader = get_continual_test_loaders_generic(
            seen_categories, data_root, image_size, batch_size, dataset_kind=dataset
        )
        ...
        for batch in tqdm(test_loader, desc="Testing (UCAD-style)"):
            imgs = batch["image"].to(DEVICE)
            labels = batch["label"].cpu().numpy()
            gt_masks = batch["mask"].squeeze(1).cpu().numpy()
            paths = batch["path"]

            img_scores, maps = infer_batch_anomaly_ucad(
            ...
            for i, p in enumerate(paths):
                cat = _true_category_from_path(p, known_categories=seen_categories)
                per_task[cat]["y_img"].append(int(labels[i]))
                per_task[cat]["s_img"].append(float(img_scores[i].item()))
                per_task[cat]["y_pix"].append(gt_masks[i])
                per_task[cat]["s_pix"].append(maps_up[i].cpu().numpy())
        ...
        fm_img_paper   = _fm_paper_eq7(hist_img_rows)
        fm_img_clamped = _fm_clamped(hist_img_rows)
        fm_pix_paper   = _fm_paper_eq7(hist_pix_rows)
        fm_pix_clamped = _fm_clamped(hist_pix_rows)

        print(f"\n--- Results after task '{category}' ({len(seen_categories)} seen) ---")
        print(f"  Image AUROC: {image_auroc:.6f}")
        print(f"  Pixel AUROC: {pixel_auroc:.6f}")
        print(f"  Pixel AUPR (all pixels): {pixel_aupr_all:.6f}")
        print(f"  FM (Image AUROC, paper Eq.7): {fm_img_paper:.6e}")
        print(f"  FM (Image AUROC, clamped)  : {fm_img_clamped:.6e}")
        print(f"  FM (Pixel AUPR,  paper Eq.7): {fm_pix_paper:.6e}")
        print(f"  FM (Pixel AUPR,  clamped)  : {fm_pix_clamped:.6e}")
        print("-" * 50)
...
if __name__ == "__main__":
    DATA_ROOT = "/Users/lenguyenlinhdan/Downloads/FSCIL_TCDS/VisA_20220922"      # <-- CHANGE ME
    ...
    run_ucad(
        ...
        use_multiscale=True,      # fuse final+early by max
        metric='euclidean',
        dataset='visa',          # 'mvtec' or 'visa'
    )
