...
def train_model(train_loader, feature_extractor, memory_bank_images=None, num_epochs=10, temperature=0.1):
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        for batch in train_loader:
            ...
            if memory_bank_images:
                rehearsal_batch_size = images.shape[0] // 2
                if rehearsal_batch_size > 0 and len(memory_bank_images) >= rehearsal_batch_size:
                    rehearsal_indices = random.sample(range(len(memory_bank_images)), rehearsal_batch_size)
                    rehearsal_images = [memory_bank_images[i] for i in rehearsal_indices]
                    
                    combined_images = torch.cat([images, torch.stack(rehearsal_images).to(DEVICE)], dim=0)
                else:
            ...
            combined_features = feature_extractor(combined_images)
            
            num_fast_prompts = 16
            num_slow_prompts = 16
            
            fast_prompts = combined_features[:, :num_fast_prompts, :]
            slow_prompts = combined_features[:, num_fast_prompts:num_fast_prompts + num_slow_prompts, :]
            patch_features = combined_features[:, num_fast_prompts + num_slow_prompts:, :]
            
            all_features = torch.cat([fast_prompts.reshape(-1, fast_prompts.shape[-1]), slow_prompts.reshape(-1, slow_prompts.shape[-1]), patch_features.reshape(-1, patch_features.shape[-1])], dim=0)
            all_features_norm = F.normalize(all_features, p=2, dim=1)
            similarity_matrix = torch.matmul(all_features_norm, all_features_norm.T)
            
            num_prompts = num_fast_prompts + num_slow_prompts
            num_patches = patch_features.shape[1]
            
            positive_mask = torch.zeros_like(similarity_matrix, dtype=torch.bool).to(DEVICE)
            positive_mask[:num_prompts, num_prompts:] = True
            positive_mask[num_prompts:, :num_prompts] = True
            
            exp_similarity = torch.exp(similarity_matrix / temperature)
            denominator = torch.sum(exp_similarity, dim=1, keepdim=True)
            
            numerator = torch.sum(exp_similarity[:num_prompts] * positive_mask[:num_prompts], dim=1)
            
            total_loss = -torch.log(numerator / denominator[:num_prompts].squeeze()).mean()
            
            total_loss.backward()
            
            optimizer.step()

def build_memory_bank(train_loader, feature_extractor, max_size):
    ...
    with torch.no_grad():
        for batch in tqdm(train_loader, desc="Building Current Task Memory"):
            images = batch["image"].to(DEVICE)
            combined_features = feature_extractor(images)
            for features, img in zip(combined_features, images):
                features_list.append(features.view(-1, features.shape[-1]))
                images_list.append(img)
    
    full_features = torch.cat(features_list, dim=0)

    if full_features.shape[0] <= max_size:
        return full_features, images_list
        
    features_np = full_features.cpu().numpy()
    
    first_idx = np.random.randint(features_np.shape[0])
    coreset_indices = [first_idx]
    coreset = features_np[first_idx:first_idx+1]
    
    for _ in tqdm(range(max_size-1), desc="Building Coreset (k-Center Greedy)"):
        dists = euclidean_distances(features_np, coreset)
        min_dists = np.min(dists, axis=1)
        farthest_idx = np.argmax(min_dists)
        coreset_indices.append(farthest_idx)
        coreset = np.vstack([coreset, features[farthest_idx]])
        
    coreset_features = full_features[coreset_indices]
    coreset_images = [images_list[i] for i in coreset_indices]

    return coreset_features, coreset_images

def find_optimal_threshold_and_f1(scores: np.ndarray, labels: np.ndarray) -> float:
    """Finds the optimal threshold that maximizes the F1-score."""
    if len(np.unique(labels)) < 2:
        return 0.0 # Cannot compute F1-score with a single class
    
    thresholds = np.unique(scores)
    f1_scores = [f1_score(labels, scores >= t) for t in thresholds]
    
    optimal_f1 = np.max(f1_scores) if f1_scores else 0.0
    return optimal_f1
...
def test_patchcore_sam(
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing"):
            ...
            combined_features = feature_extractor(images)
            B, N_PROMPT_AND_PATCHES, C = combined_features.shape
            
            patch_features = combined_features[:, 32:, :]
            ...
            side = int(math.sqrt(patch_features.shape[1]))
    ...
    # Filter for anomaly masks only for pixel metrics
    gt_masks_flat = gt_masks.flatten()
    pixel_scores_flat = pixel_scores.flatten()
    
    gt_masks_flat = gt_masks_flat[gt_labels.repeat(256 * 256) == 1]
    pixel_scores_flat = pixel_scores_flat[gt_labels.repeat(256 * 256) == 1]
    
    pixel_auroc = roc_auc_score(gt_masks_flat, pixel_scores_flat)

    # Calculate F-measure
    image_f_measure = find_optimal_threshold_and_f1(image_scores, gt_labels)
    pixel_f_measure = find_optimal_threshold_and_f1(pixel_scores_flat, gt_masks_flat)
...
def run_continual_learning(
    ...
    for task_idx, category in enumerate(categories):
        ...
        train_model(train_loader, feature_extractor, combined_memory_images)
        
        current_task_features, current_task_images = build_memory_bank(
            train_loader, feature_extractor, max_memory_bank_size
        )

        if combined_memory_bank is None:
            combined_memory_bank = current_task_features
            combined_memory_images = current_task_images
        else:
            combined_memory_bank = torch.cat([combined_memory_bank, current_task_features], dim=0)
            combined_memory_images.extend(current_task_images)
        
        if len(combined_memory_images) > max_memory_bank_size:
            combined_features_np = combined_memory_bank.cpu().numpy()
            indices = k_center_greedy_coreset(combined_features_np, max_memory_bank_size)
            combined_memory_bank = combined_memory_bank[indices]
            combined_memory_images = [combined_memory_images[i] for i in indices]
        ...
        (gt_labels, image_scores, gt_masks, pixel_scores, image_auroc, pixel_auroc, image_f_measure, pixel_f_measure) = test_patchcore_sam(
        ...
        print(f"Image-level F-measure: {image_f_measure:.4f}")
        print(f"Pixel-level AUROC: {pixel_auroc:.4f}")
        print(f"Pixel-level F-measure: {pixel_f_measure:.4f}")

def k_center_greedy_coreset(features: np.ndarray, k: int) -> np.ndarray:
    if features.shape[0] <= k:
        return np.arange(features.shape[0])
    
    first_idx = np.random.randint(features.shape[0])
    coreset_indices = [first_idx]
    coreset = features[first_idx:first_idx+1]
    
    for _ in tqdm(range(k-1), desc="Building Coreset (k-Center Greedy)"):
        dists = euclidean_distances(features, coreset)
        min_dists = np.min(dists, axis=1)
        farthest_idx = np.argmax(min_dists)
        coreset_indices.append(farthest_idx)
        coreset = np.vstack([coreset, features[farthest_idx]])
